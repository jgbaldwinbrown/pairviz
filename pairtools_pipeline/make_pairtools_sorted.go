...
func AddSplit(mf *makem.MakeData, name string, nsplits int, indir, outdir, scriptdir string) {
	...
	r.AddScripts(
		"mkdir -p `dirname $@`",
		CleanSlash(scriptdir + "/split.sh 32 " + deps[0] + " " + outdir + "/fq_split/" + name + "_R1_001_split_ 125530532"),
		CleanSlash(scriptdir + "/split.sh 32 " + deps[1] + " " + outdir + "/fq_split/" + name + "_R2_001_split_ 125530532"),
		"touch $@",
	)
	mf.Add(r)
}
...
func AddTrim(mf *makem.MakeData, name string, nsplits int, outdir, scriptdir string) {
	...
	for i:=0; i<nsplits; i++ {
		...
		r1 := CleanSlash(fmt.Sprintf("%v/%v_R1_001_split_%04d.fastq.gz", splitdir, name, i))
		r2 := CleanSlash(fmt.Sprintf("%v/%v_R2_001_split_%04d.fastq.gz", splitdir, name, i))
		r.AddScripts(
			"mkdir -p `dirname $@`",
...
func AddBwa(mf *makem.MakeData, name string, nsplits int, ref, refdir, outdir, scriptdir string) {
	...
	for i:=0; i<nsplits; i++ {
		...
		r1 := CleanSlash(fmt.Sprintf("%v/%v_R1_001_split_%04d.fastq.gz_ftrimmed.fq.gz", trimdir, name, i))
		r2 := CleanSlash(fmt.Sprintf("%v/%v_R1_001_split_%04d.fastq.gz_rtrimmed.fq.gz", trimdir, name, i))
		r.AddScripts(
			"mkdir -p `dirname $@`",
...
func AddMerge(mf *makem.MakeData, name string, nsplits int, outdir string) {
	...
	r.AddScripts(
		"mkdir -p `dirname $@`",
		CleanSlash("samtools merge - " + inglob + " | samtools sort -n > " + output),
		"touch $@",
	)
	mf.Add(r)
}
...
func MakeSplitsCluster(p Params) error {
	path := fmt.Sprintf("runscripts/run_%v_splits_cluster.sh", p.Name)
	w, err := os.Create(path)
	if err != nil {
		return err
	}
...
func MakeStarts(p Params) error {
	...
	target2 := CleanSlash(p.Outdir + "/fq_split/split.done")
